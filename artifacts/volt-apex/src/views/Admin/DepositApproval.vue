<template>
  <div class="min-h-screen" style="background: var(--volt-black);">
    <div class="sticky top-0 z-50 px-4 h-14 flex items-center gap-3" style="background: rgba(10,14,26,0.95); backdrop-filter: blur(20px); border-bottom: 1px solid rgba(0,212,255,0.1);">
      ...
      <span class="ml-auto text-xs px-2 py-1 rounded-full font-bold" style="background: rgba(255,61,90,0.15); color: var(--volt-red);">{{ txStore.pendingDeposits.length }} ဆောင်ရွက်ရန်</span>
    </div>
    ...
    <div class="px-4 pt-4 pb-8 max-w-lg mx-auto">
      ...
      <div v-if="txStore.loading" class="space-y-3">
        <VoltSkeleton v-for="i in 4" :key="i" height="180px" radius="14px" />
      </div>
      ...
      <div v-else class="space-y-3">
        <div v-for="t in displayList" :key="t.id" class="glass-card relative overflow-hidden">
          ...
          <div class="grid grid-cols-2 gap-2 mb-3">
            <div class="p-2 rounded-xl" style="background: rgba(5,8,17,0.6);">
              ...
              <div class="font-number text-base font-black text-gold-gradient">{{ t.amount.toLocaleString() }} K</div>
            </div>
            <div class="p-2 rounded-xl" style="background: rgba(5,8,17,0.6);">
              <div class="text-xs mb-0.5" style="color: var(--volt-gray);">နည်းလမ်း</div>
              <div class="text-sm font-medium" style="color: var(--volt-electric);">{{ t.payment_method?.toUpperCase() }}</div>
            </div>
          </div>

          <div v-if="t.payment_ref" class="p-2 rounded-xl mb-3 text-xs" style="background: rgba(5,8,17,0.6);">
            <span style="color: var(--volt-gray);">Ref: </span><span style="color: var(--volt-white);">{{ t.payment_ref }}</span>
          </div>

          <div v-if="t.screenshot_url" class="mb-3 cursor-pointer" @click="viewScreenshot(t.screenshot_url)">
            <img :src="t.screenshot_url" class="w-full max-h-36 object-cover rounded-xl" loading="lazy" />
            <div class="text-xs text-center mt-1" style="color: var(--volt-electric);">🔍 tap ကြည့်ရန်</div>
          </div>
          ...
          <div v-if="t.status === 'pending'" class="flex gap-2">
            <button class="flex-1 h-10 rounded-xl text-sm font-bold" style="background: rgba(0,255,179,0.1); color: var(--volt-emerald); border: 1px solid rgba(0,255,179,0.25);" :disabled="processing === t.id" @click="approve(t.id)">
              <span v-if="processing === t.id" class="text-xs">⏳ ...</span>
              <span v-else>✓ Approve</span>
            </button>
            <button class="flex-1 h-10 rounded-xl text-sm font-bold" style="background: rgba(255,61,90,0.1); color: var(--volt-red); border: 1px solid rgba(255,61,90,0.25);" @click="openReject(t.id)">
              ✕ Reject
            </button>
          </div>
        </div>
        ...
        <div v-if="displayList.length === 0" class="text-center py-12">
          ...
          <p class="text-sm" style="color: var(--volt-gray);">ဆောင်ရွက်ရမည့် Deposit မရှိပါ</p>
        </div>
      </div>
    </div>

    <!-- Reject Modal -->
    <VoltModal v-model="showRejectModal" title="Reject ကြောင်း ပြောပါ">
      <div>
        <textarea v-model="rejectNote" rows="3" placeholder="Reject ကြောင်းရင်း (ရွေးချယ်)..."
          class="w-full p-3 rounded-xl text-sm outline-none resize-none"
          style="background: rgba(5,8,17,0.8); border: 1px solid rgba(0,212,255,0.2); color: var(--volt-white);"></textarea>
      </div>
      <template #footer>
    ...
    <!-- Screenshot Viewer -->
    <div v-if="screenshotView" class="fixed inset-0 z-50 flex items-center justify-center p-4" style="background: rgba(5,8,17,0.95);" @click="screenshotView = ''">
      <img :src="screenshotView" class="max-w-full max-h-full rounded-xl object-contain" />
    </div>
  </div>
</template>
...
import type { TxnStatus, Transaction } from "@/stores/transactions.store";
import { supabase } from "@/lib/supabase";

const txStore = useTransactionsStore();
const filter = ref<TxnStatus | "all">("pending");
const processing = ref("");
const showRejectModal = ref(false);
const rejectNote = ref("");
const rejectTargetId = ref("");
const screenshotView = ref("");
const allDeposits = ref<any[]>([]);
...
const displayList = computed(() => {
  if (filter.value === "pending") return txStore.pendingDeposits;
  return allDeposits.value.filter(t => t.status === filter.value);
});

async function setFilter(f: TxnStatus | "all") {
  filter.value = f as TxnStatus;
  if (f !== "pending") {
    const { data } = await supabase
      .from("transactions")
      .select("*, users(username, phone_number)")
      .eq("type", "deposit").eq("status", f).order("created_at", { ascending: false }).limit(30);
    if (data) allDeposits.value = data;
  }
}

async function approve(id: string) {
  processing.value = id;
  await txStore.approveDeposit(id);
  processing.value = "";
}
...
async function confirmReject() {
  await txStore.rejectTransaction(rejectTargetId.value, rejectNote.value);
  showRejectModal.value = false;
}
function viewScreenshot(url: string) { screenshotView.value = url; }

function formatDate(d: string) {
  return new Date(d).toLocaleString("my-MM", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" });
}
function statusStyle(s: TxnStatus) {
  const m: Record<string, string> = {
    ...
    processing: "background: rgba(0,212,255,0.12); color: var(--volt-electric);",
  };
  return m[s] || m.pending;
}
function statusLabel(s: TxnStatus) {
  return { approved: "✓ Approved", pending: "⏳ Pending", rejected: "✕ Rejected", processing: "⚙ Processing" }[s] || s;
}

onMounted(() => txStore.fetchPendingDeposits());
</script>
